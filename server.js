  ...
  try {
    ...
    if (secret && secret.startsWith('sk_')) {
      try {
        ...
        // Invalid key / failed resolve → fall back to demo (do not block signup)
        console.log('Paystack resolve failed, using demo:', data.message || data);
      } catch (err) {
        console.log('Paystack error, using demo:', err.message);
      }
    }

    // Demo / fallback — always succeeds so registration is not blocked
